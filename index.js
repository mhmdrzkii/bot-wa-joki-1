...
const { smsg, isUrl, generateMessageTag, getBuffer, getSizeMedia, fetchJson, await, sleep, jsonformat } = require('./lib/myfunc')
...
const { checkSewaGrup, expiredCheck } = require('./lib/sewa')
...
`.trim(), anu)
        ...
        try {
            // console.log(tio.user)
            const str = tio.user.id
            const match = str.match(/^(\d+):/);
            const botId = match.length ? match[1] : null;
            // console.log(botId)
            // const botId = tio.user.id.replace(":21", "")
