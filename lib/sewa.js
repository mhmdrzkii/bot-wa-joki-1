...
const checkSewaGrup = (groupId, _dir) => {
  ...
  Object.keys(_dir).forEach((i) => {
    ...
    if (Date.now() >= _dir[i].expired) {
      const newData = _dir.filter((group) => group.id !== groupId)
      fs.writeFileSync("./database/sewa.json", JSON.stringify(newData))
      status = false
    }
  })
